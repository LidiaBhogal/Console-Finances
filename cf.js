...
for (let i = 0; i < finances.length; i++) {
    ...
    if (finances[i + 1]) {
        console.log(mostProfit, monthIncrease)
        monthlyChanges.push(finances[i + 1][1] - finances[i][1]);
        if ((finances[i + 1][1] - finances[i][1]) > monthIncrease) {
            monthIncrease= finances[i + 1][1] - finances[i][1];
            mostProfit = finances[i+1]
        }
        if (finances[i + 1][1] - finances[i][1] < monthDecrease) {
            monthDecrease = finances[i + 1][1] - finances[i][1];
            leastProfit= finances[i+1]
        }
...
for (let i = 0; i < monthlyChanges.length; i++) {
    ...
    monthlyDifference+=monthlyChanges[i]
}
